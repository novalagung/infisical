...
| Field | Type | Description | Default value<br>(if not specified) | Required/optional |
| :- | :-: | :- | :-: | :-: |
| `SiteUrl` | string | The URL of the Infisical API | `https://app.infisical.com` | optional |
| `UserAgent` | string | Set the user agent that will be used for HTTP requests. Do not modify this unless you have a reason to do so. | `infisical-go-sdk` | optional |
| `AutoTokenRefresh` | boolean | Whether or not to let the SDK handle the access token lifecycle | `true` | optional |
| `SilentMode` | boolean | Whether or not to suppress logs such as warnings from the token refreshing process | `false` | optional |
